...
public final class Matrixes {
	...
	public static double[] row_mul(double[] x, double[] y) throws MyException {
		...
		if (x.length != y.length)
			throw new MyException("Cannot multiply vectors el by el. Vectors must have same length, while it is ["
					+ Integer.toString(x.length) + "] and [" + Integer.toString(y.length) + "].");
		else {
			double[] temp = new double[x.length];
			for (int i = 0; i < x.length; i++) {
				temp[i] = x[i] * y[i];
	...
	public static double[][] multiplyByMatrix(double[][] m1, double[][] m2) throws MyException {
		int m1ColLength = m1[0].length; // columns length
		int m2RowLength = m2.length; // rows length
		if (m1ColLength != m2RowLength) {
			throw new MyException("While multiplying matrixes, number of columns of first array ["
					+ Integer.toString(m1ColLength) + "] must be the same as number of rows in second array ["
					+ Integer.toString(m2RowLength) + "]. Obviously, it is not.");
		}
	...
	public static double[] multiplyByMatrix(double[][] m1, double[] m2) throws MyException {
		...
		if (m1ColLength != m2RowLength) 
			throw new MyException("While multiplying matrix by vector, number of columns of first array ["
					+ Integer.toString(m1ColLength)
					+ "] must be the same as number of rows (elements) in second vector ["
					+ Integer.toString(m2RowLength) + "]. Obviously, it is not.");
		int mRRowLength = m1.length; 
		int mRColLength = m2RowLength; 
	...
	public static double[][] multiplyMatrixesElByEl(double[][] m1, double[][] m2) throws MyException {
		...
		if (m1.length != m2.length || m1[0].length != m2[0].length) {
			throw new MyException(
					"While multiplying matrixex element by element, they must have equal dimmensions, while it is ["
							+ Integer.toString(m1.length) + "][" + Integer.toString(m1[0].length) + "] and ["
							+ Integer.toString(m2.length) + "][" + Integer.toString(m2[0].length) + "].");
		}
	...
	public static double[][] multiplyByValue(double[][] x, double y[]) throws MyException {
		...
		if (x.length != y.length && x[0].length != y.length)
			throw new MyException(
					"Cannot multiply matrix by vecror element by element, neither row-wise nor column-wise. Number of elements in vector ["
							+ Integer.toString(y.length)
							+ "] must be equal to any of dimmension parameters of first array ["
							+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "].");
	...
	public static double[][] substractValue(double[][] x, double y[]) throws MyException {
		...
		if (x.length != y.length && x[0].length != y.length)
			throw new MyException(
					"Cannot substract vecror from array element by element, neither row-wise nor column-wise. Number of elements in vector ["
							+ Integer.toString(y.length)
							+ "] must be equal to any of dimmension parameters of first array ["
							+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "].");
		double[][] temp = new double[x.length][x[0].length];
	...
	public static double[][] addValue(double[][] x, double y[]) throws MyException {
		...
		if (x.length != y.length && x[0].length != y.length)
			throw new MyException(
					"Cannot add vecror to array element by element, neither row-wise nor column-wise. Number of elements in vector ["
							+ Integer.toString(y.length)
							+ "] must be equal to any of dimmension parameters of first array ["
							+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "].");
	...
	public static double[] addMatrixes(double[] x, double[] y) throws MyException {
		...
		if (x.length != y.length)
			throw new MyException("Cannot add vectors el by el. Vectors must have same length, while it is ["
					+ Integer.toString(x.length) + "] and [" + Integer.toString(y.length) + "].");
	...
	public static double[][] addMatrixes(double[][] x, double[][] y) throws MyException {
		...
		if (x.length != y.length || x[0].length != y[0].length) {
			throw new MyException(
					"While adding matrixes element by element, they must have equal dimmensions, while it is ["
							+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "] and ["
							+ Integer.toString(y.length) + "][" + Integer.toString(y[0].length) + "].");
		}
	...
	public static double[][] substractMatrixes(double[][] x, double[][] y) throws MyException {
		...
		if (x.length != y.length || x[0].length != y[0].length) {
			throw new MyException(
					"While substracting matrixes element by element, they must have equal dimmensions, while it is ["
							+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "] and ["
							+ Integer.toString(y.length) + "][" + Integer.toString(y[0].length) + "].");
		}
	...
	public static double[] minimum(double[] x, double[] y) throws MyException {
		double[] temp = new double[x.length];
		if (x.length != y.length)
			throw new MyException("Cannot search minimum value from two vectors of different length - ["
					+ Integer.toString(x.length) + "] and [" + Integer.toString(y.length) + "].");
		for (int i = 0; i < x.length; i++) {
			if (y[i] < x[i]) {
				temp[i] = y[i];
	...
	public static double[] select_row(double[][] x, int y) throws MyException {
		...
		if (y > x.length - 1)
			throw new MyException("Selected row out of range - " + Integer.toString(y) + ". of "
					+ Integer.toString(x.length) + " rows (remember aobout 0th row!).");
	...
	public static double[][] cov(double[][] x) {
		...
		try {
			...
			temp = Matrixes.divideByValue(Matrixes.multiplyByMatrix(x, Matrixes.transpose(temp)),
					(double) x[0].length - 1);
		} catch (Exception myEx) {
			System.out.println("An exception encourred: " + myEx.getMessage());
			myEx.printStackTrace();
	...
	public static double[] makeLog(double[] x) throws MyException {
		...
		for (int i = 0; i < x.length; i++) {
			...
			if (x[i] <= 0)
				throw new MyException("Cannot make Log of value below 0 - Log(" + Double.toString(x[i]) + "), (index "
						+ Integer.toString(i) + ").");
			temp[i] = Math.log(x[i]);
...
//做log
	public static double[][] makeLog(double[][] x) throws MyException {
		...
		for (int i = 0; i < x.length; i++) {
			for (int j = 0; j < x[0].length; j++) {
				...
				if (x[i][j] <= 0)
					throw new MyException("Cannot make Log of value below 0 - Log(" + Double.toString(x[i][j])
	...
	public static double[] invertElements(double[] x) throws MyException {
		...
		for (int i = 0; i < x.length; i++) {
			if (x[i] == 0)
				throw new MyException(
						"While inverting values, cannot divide by 0, (index " + Integer.toString(i) + ").");
			temp[i] = 1 / (x[i]);
		}
		return temp;
	}
	...
	public static double[][] divideElements(double[][] x, double[][] y) throws MyException {
		...
		if (x.length != y.length || x[0].length != y[0].length) {
			throw new MyException("While dividing element by element, they must have equal dimmensions, now it is ["
					+ Integer.toString(x.length) + "][" + Integer.toString(x[0].length) + "] and ["
					+ Integer.toString(y.length) + "][" + Integer.toString(y[0].length) + "].");
		}
		...
		for (int i = 0; i < y.length; i++) {
			for (int j = 0; j < x[0].length; j++) {
				if (y[i][j] <= 0)
					throw new MyException("While inverting values, cannot divide by 0 (y[" + Integer.toString(i) + "]["
							+ Integer.toString(j) + "]).");
