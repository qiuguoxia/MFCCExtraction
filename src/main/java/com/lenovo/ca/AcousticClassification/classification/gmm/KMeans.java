...
public class KMeans {
	...
	public void fit() {
		...
		try {
			...
			for (int i = 0; i < this.n_init; i++) {
				Kmeans_single kmeans_single = new Kmeans_single(this.data, this.numOfClusters, x_squared_norms,
						this.max_iter, this.tol);
	...
	private class LabelsInertia {
		...
		LabelsInertia(double[][] X, double[] x_squared_norms, double[][] centers, double[] distances) {
			...
			LabelsPrecomputeDence result = new LabelsPrecomputeDence(this.data, this.x_squared_norms, this.centers,
					this.distances);
