...
public class Filtering {
    public static double[] filterSignal(double[] signal, double sampleRate ,double cutoffFreq, double filterOrder, int filterType, double ripplePercent) {
        double[][] recursionCoefficients = new double[22][2];
        // Generate double array for ease of coding
        double[] unfilteredSignal =   new double[signal.length];
        for (int i=0; i<signal.length; i++) {
            unfilteredSignal[i] =   signal[i];
        }
        ...
        for (int i=0; i<20; i++){
            ...
            if (filterType==1) sB   =   sB+coeffB[i]*Math.pow(-1,i);
        }
    ...
    */
    ...
    static double[] MakeFilterParameters(double cutoffFraction, int filterType, double rippleP, double numberOfPoles, int iteration) {
...
//        }
    ...
    /**
     * The bandpass filter combined from the lowpass filter and highpass filter.
     * @return
     */
    public static double[] bandpass(double[] signal, double sampleRate, double lowCut, double highCut, double filterOrder, double ripplePercent) {
        // Pass the low pass filter
        double[] signalLowpass = filterSignal(signal, sampleRate, highCut, filterOrder, 0, ripplePercent);
        // Pass the high pass filter
        double[] signalBandpass = filterSignal(signalLowpass, sampleRate, lowCut, filterOrder, 1, ripplePercent);
        return signalBandpass;
    }

    public static void main(String[] args) {
        double[] data = {};
        System.out.println(Arrays.toString(bandpass(data, 8000, 0, 100, 4, 0)));
